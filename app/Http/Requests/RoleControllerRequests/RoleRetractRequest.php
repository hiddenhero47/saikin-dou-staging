...
class RoleRetractRequest extends FormRequest
{
    /**
    ...
    public function authorize()
    {
         ...
         * Check if the user is an administrator with permission of retract_role
         */
        if ($this->header('Team') === 'administrator' && auth()->user()->isAbleTo('retract_role', 'administrator')) {
            return true;
        }

        /**
         * Check if requestor is able to ...
         * Check if the user is a manager with permission of retract_role
         */
        if ($this->header('Team') === 'management' && auth()->user()->isAbleTo('retract_role', 'management')) {
         ...
         */
        if (auth()->user()->isAbleTo('retract_role', $this->header('Team'))) {
            return $this->merge(['team_name' => $this->header('Team')]);
        }
