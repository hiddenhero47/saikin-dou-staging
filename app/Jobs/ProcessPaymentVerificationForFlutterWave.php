...
class ProcessPaymentVerificationForFlutterWave implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // Verify payment
        $FlutterWaveSuite = FlutterWaveSuite::client()
        ->setSecretKey(config('ov.flutter_wave_secret_key'))
        ->setPaymentVerificationCriteria($payment->id, $payment->amount, $payment->currency)
        ->verifyPayment(config('ov.flutter_wave_client_url_payment_verification'),$this->id);

        // Update payment for successful payments
        if ($FlutterWaveSuite->authentic() && $FlutterWaveSuite->response()->data->status === 'successful'){

            $payment->paid = true;
            $payment->confirmed = true;
            $payment->status = config('constants.payment.status.success');
            $payment->method = config('constants.payment.provider.flutterwave');
            $payment->details = json_encode($FlutterWaveSuite->response());
            $payment->reference = $FlutterWaveSuite->response()->data->id;
            $payment->type = $FlutterWaveSuite->response()->data->meta->payment_type ?? config('constants.payment.type.standard');
        ...
        if (!$FlutterWaveSuite->authentic() || $FlutterWaveSuite->response()->data->status === 'failed'){
            ...
            $payment->method = config('constants.payment.provider.flutterwave');
            $payment->details = json_encode($FlutterWaveSuite->response());
            $payment->reference = $FlutterWaveSuite->response()->data->id ?? null;
            $payment->type = $FlutterWaveSuite->response()->data->meta->payment_type ?? config('constants.payment.type.standard');
    ...
    public function failed($exception)
    {
        if (config('ov.api_exception_report', false)) {
            ...
            DiscordSuite::messenger()->setWebhook(config('ov.discord_webhook_url'))->sendWebhook([
                ...
                'Location' => 'ProcessPaymentVerificationForFlutterWave',
            ]);
        }
    }
}
